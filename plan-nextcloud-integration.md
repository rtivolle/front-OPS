...
- [ ] Implémenter un nouvel endpoint `/summarize?id=...` dans **doc-worker** qui génère un résumé pour un document donné
- [ ] Utiliser un modèle CPU (3–8B, ex. llama.cpp) pour générer des résumés courts

#### 4. Renforcement de la sécurité
- [ ] Créer un `app password` dédié pour le compte bot dans Nextcloud
- [ ] Activer TLS pour les communications internes entre les conteneurs
- [ ] Configurer une liste blanche d’adresses IP autorisées à accéder au système
- [ ] Restreindre les permissions de Flow aux utilisateurs du groupe test uniquement
